...
def fillObsCPT(bayesNet, gameState):
    ...
    """
    "*** YOUR CODE HERE ***"
    def getadjacenthouse(gameState,obsPos):
        mindistance=float("inf")
        adjacentHouse=None
        for house in gameState.getPossibleHouses():
            distance=util.manhattanDistance(obsPos,house)
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            for assignment in obsVarFactor.getAllPossibleAssignmentDicts():
                ...
                adjacentHouse=getadjacenthouse(gameState,obsPos)
                prob=0
                if adjacentHouse!=ghosthouseVal and adjacentHouse!=foodhouseVal:
                    if color==RED_OBS_VAL:
                        ...
                        prob=1
                elif ghosthouseVal==foodhouseVal or adjacentHouse==foodhouseVal:
                    if color==RED_OBS_VAL:
                        prob=PROB_FOOD_RED
                    elif color==BLUE_OBS_VAL:
                        ...
                        prob=0
                elif adjacentHouse==ghosthouseVal:
                    if color==RED_OBS_VAL:
                        prob=PROB_GHOST_RED
                    elif color==BLUE_OBS_VAL:
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    "*** YOUR CODE HERE ***"
    factorFoodHouse=inference.inferenceByVariableElimination(bayesNet,['foodHouse'],evidence,eliminationOrder)
    maxprob=float('-inf')
    pos=()
    for assignment in factorFoodHouse.getAllPossibleAssignmentDicts():
        prob=factorFoodHouse.getProbability(assignment)
        if prob>maxprob:
           maxprob=prob
           pos=assignment
    return pos
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        "*** YOUR CODE HERE ***"
        factor=inference.inferenceByVariableElimination(self.bayesNet,['foodHouse','ghostHouse'],evidence,eliminationOrder)
        prob1=0
        prob2=0
        ...
        leftExpectedValue=prob1*WON_GAME_REWARD+(1-prob1)*GHOST_COLLISION_REWARD
        rightExpectedValue=prob2*WON_GAME_REWARD+(1-prob2)*GHOST_COLLISION_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        list1=self.getExplorationProbsAndOutcomes(evidence)
        for prob, explorationEvidence in list1:
