...
def joinFactors(factors):
    ...
    for factor in factors:
        for variable in factor.conditionedVariables():
            if variable not in listofConditioned and variable not in listofUnConditioned:
               listofConditioned.append(variable)
    result=Factor(listofUnConditioned,listofConditioned,variableDomainsDict)
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor, eliminationVariable):
        ...
        for assignment in factor.getAllPossibleAssignmentDicts():
            result.setProbability(assignment,factor.getProbability(assignment)+result.getProbability(assignment))
        return result
    ...
    if total==0:
        ...
        for variable in variableDomainsDict:
            if len(variableDomainsDict[variable])==1:
                if variable in unconditionedVariables:
                    unconditionedVariables.remove(variable)
                    conditionedVariables.add(variable)
        result=Factor(unconditionedVariables,conditionedVariables,variableDomainsDict)
        for assignment in factor.getAllPossibleAssignmentDicts():
            result.setProbability(assignment,factor.getProbability(assignment)/total)
    return result
