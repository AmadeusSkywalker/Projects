...
class StudentUSocket(StudentUSocketBase):
  ...
  def close(self):
    ...
    if self.state is CLOSED:
      ...
      control = FinControl(self)
      control.set_pending("FIN_WAIT_1")
  ...
  def rx(self, p):
    ...
    * Send pending ACKs and FINs
    ...
    # Complete for Stage 3 (check recv queue)
      if (not self.rx_queue.empty() and self.rx_queue.peek()[0] |GT| self.rcv.nxt):
        self.set_pending_ack()
      else:
        while not self.rx_queue.empty() and not self.rx_queue.peek()[0] |GT| self.rcv.nxt:
  ...
  def update_rto(self, acked_pkt):
    ...
    if(self.rto == 1):
      ...
      rttprime = self.stack.now - acked_pkt.tx_ts
      self.rttvar = (1 - self.beta) * self.rttvar + self.beta * abs(self.srtt - rttprime)
      self.srtt = (1 - self.alpha) * self.srtt + self.alpha * rttprime
      self.rto = self.srtt + max(self.G, self.K * self.rttvar)
  ...
  def update_window(self, seg):
    ...
    self.snd.wnd = seg.win # remove w/ implemented
  ...
  def handle_accepted_fin(self, seg):
    ...
    if(self.state == FIN_WAIT_1):
      control = FinControl(self)
      if(control.acks_our_fin(seg.ack)):
        self.state = TIME_WAIT
      else:
        self.state = CLOSING
    if self.state == FIN_WAIT_2:
      control = FinControl(self)
      if(not control.acks_our_fin(seg.ack)):
        self.state = TIME_WAIT
  ...
  def maybe_send(self):
    ...
    bytes_sent = 0
    # Complete for Stage 4
    remaining = min(len(self.tx_data),self.snd.wnd-(self.snd.nxt-self.snd.una))
    while remaining > 0:
      minval = min(self.mss, self.snd.wnd-(self.snd.nxt-self.snd.una),len(self.tx_data))
      payload = self.tx_data[0:minval]
      self.tx_data = self.tx_data[minval:]
      remaining -= minval
      packet = self.new_packet()
