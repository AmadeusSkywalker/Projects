...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        # BEGIN Problem 3B
        count=0
        if len(self.place.bees)!=0 and (count>=self.min_range or count<=self.max_range):
            return random_or_none(self.place.bees)
        else:
            beeplace=self.place.entrance
            count=count+1
            while beeplace!=hive:
                if len(beeplace.bees) !=0 and (count>=self.min_range or count<=self.max_range):
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        if self.imposter==True:
            ...
            while place!=None:
                if place.ant!=None:
                    if place.ant.container==False:
                        if place.ant not in self.braveants:
                            place.ant.damage=place.ant.damage*2
                            self.braveants.append(place.ant)
                    else:
                        if place.ant not in self.braveants:
                                place.ant.damage=place.ant.damage*2
                                self.braveants.append(place.ant)
                                if place.ant.ant!=None and place.ant.ant not in self.braveants:
                                    place.ant.ant.damage=place.ant.ant.damage*2
                                    self.braveants.append(place.ant.ant)
                        else:
                            if place.ant.ant!=None and place.ant.ant not in self.braveants:
                                  place.ant.ant.damage=place.ant.ant.damage*2
                                  self.braveants.append(place.ant.ant)
