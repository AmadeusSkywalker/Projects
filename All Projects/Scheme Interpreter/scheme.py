...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operands=rest.map(lambda evaluated: scheme_eval(evaluated,env))
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        while not formals is nil:
            if vals is nil:
                raise SchemeError('Two little values!')
            child.define(formals.first,vals.first)
            formals=formals.second
            vals=vals.second
        if not vals is nil:
            raise SchemeError('Two many values!') 
        # END PROBLEM 11
        return child
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        def wrapper(func, args):
            return func(*args)
        try:
            result=wrapper(self.fn,python_args)
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 9
    lambda1=LambdaProcedure(expressions.first,expressions.second,env)
    return lambda1
    # END PROBLEM 9
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        ...
        return scheme_eval(expressions.first,env,True)
    elif not scheme_eval(expressions.first,env) is False:
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            ...
            if not clause.second is nil:
               if clause.second.second is nil:
...
def make_let_frame(bindings, env):
    ...
    check_formals(formals)
    cenv=env.make_child_frame(formals, vals)
    return cenv
    # END PROBLEM 15
...
def do_mu_form(expressions, env):
    ...
    check_formals(formals)
    # BEGIN PROBLEM 16
    mu_form=MuProcedure(formals,expressions.second)
    return mu_form
    # END PROBLEM 16
