...
class Graph(object):
    ...
    def get_inputs(self, node):
        ...
        for parent in node.get_parents():
            result.extend([self.get_output(parent)])
        return result
    ...
    def get_output(self, node):
        ...
        """
        "*** YOUR CODE HERE ***"
        return self.output[self.inputs.index(node)]
    ...
    def get_gradient(self, node):
        ...
        """
        "*** YOUR CODE HERE ***"
        return self.gradient[self.inputs.index(node)]
    ...
    def add(self, node):
        ...
        self.inputs.extend([node])
        inp = self.get_inputs(node)
        out = node.forward(inp)
        g = np.zeros(out.shape)
        self.output.extend([out])
        self.gradient.extend([g])
    ...
    def backprop(self):
        ...
        self.gradient[inputlen-1] = 1.0
        for i in range(inputlen-1):
            index=inputlen-1-i
            currentnode=self.inputs[index]
            values=currentnode.backward(self.get_inputs(currentnode),self.gradient[index])
            for m in range(len(currentnode.get_parents())):
                parent=currentnode.get_parents()[m]
                parentindex=self.inputs.index(parent)
                self.gradient[parentindex]+=values[m]
    ...
    def step(self, step_size):
        ...
        for var in self.variables:
            ...
            var.data = var.data - np.dot(step_size, gradient)
...
class SquareLoss(FunctionNode):
    ...
    def forward(inputs):
        "*** YOUR CODE HERE ***"
        return np.mean(0.5*np.add(inputs[0],-inputs[1])**2)

    @staticmethod
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        size = len(inputs[0]) * len(inputs[0][0])
        return [gradient * np.add(inputs[0], -inputs[1]) / size, gradient * np.add(inputs[1], -inputs[0]) / size]
