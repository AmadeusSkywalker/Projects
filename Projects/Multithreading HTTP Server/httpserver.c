...
void serve_file(int fd, char *path) {
  ...
  int file_size = lseek(read_fd, (size_t)0, SEEK_END);
  lseek(read_fd, (size_t)0, SEEK_SET);
  sprintf(lenbuffer, "%d", file_size);
  
  http_start_response(fd, 200);
  http_send_header(fd, "Content-Type", http_get_mime_type(path));
  http_send_header(fd, "Content-Length", lenbuffer); // Change this too
  http_end_headers(fd);
  ...
  int bytes_read = 0;
  int buf_size = 0;
  while((bytes_read = read(read_fd, &buffer[buf_size],sizeof(buffer) - buf_size))){
      buf_size += bytes_read;
  }
  close(read_fd);
  int bytes_written = 0;
  while(buf_size > bytes_written){
      bytes_written += write(fd, &buffer[bytes_written], buf_size - bytes_written);
  }
}
...
void serve_directory(int fd, char *path) {
  ...
  */
  ...
  /* TODO: PART 3 */
  char path_buf[strlen(path) + strlen("/index.html") + 1];
  http_format_index(path_buf, path);
  struct stat sfile;
  if(stat(path_buf, &sfile) != -1){
      serve_file(fd, path_buf);
  }else{
      http_start_response(fd, 200);
      http_send_header(fd, "Content-Type", http_get_mime_type(".html"));
      http_end_headers(fd);
      ...
      while ((de = readdir(dr)) != NULL){
          ...
          char* filebuff = calloc(sizeof(char*), length);
          http_format_href(filebuff, path, filename);
          strcat(buffer, filebuff);
          finallength += length;
      } 
      int bytes_written = 0;
      while(finallength > bytes_written){
          bytes_written += write(fd, &buffer[bytes_written], finallength - bytes_written);
      }
      closedir(dr);
  }
}
...
void *proxy_work(void *arguments){
    struct arg_struct *args = arguments;
    int origin = args->origin;
    int destination = args->destination;
    char buffer[1024];
    int bytes_read = 0;
    while((bytes_read = read(origin, buffer, 1024))> 0){
...
void handle_proxy_request(int fd) {
  ...
  */
  ...
  for(int i = 0; i < 2; i++){
      if(i == 0){
         args[i].origin = fd;
         args[i].destination = target_fd;
      }else{
         args[i].origin = target_fd;
         args[i].destination = fd;
      }
      rc = pthread_create(&threads[i], NULL, &proxy_work, (void *)&args[i]);
      if (rc) {
...
void init_thread_pool(int num_threads, void (*request_handler)(int)) {
  ...
  for(int i = 0; i < num_threads; i++){
      rc = pthread_create(&threads[i], NULL, &handle_clients, (void *)request_handler);
      if (rc) {
           printf("ERROR; return code from pthread_create() is %d\n", rc);
           exit(-1);
      }
...
void *thread_handle(void *arguments){
    struct arg_struct2 *args = (struct arg_struct2 *)arguments;
    int client_fd = args->client_fd;
    args->request_handler(client_fd);
    //close(client_fd);
    pthread_exit(NULL);
}
...
void serve_forever(int *socket_number, void (*request_handler)(int)) {
  ...
  *socket_number = socket(PF_INET, SOCK_STREAM, 0);
  ...
  while (1) {
    ...
    /* PART 6 END */
     pthread_t child;
     struct arg_struct2 *arguments = malloc(sizeof(struct arg_struct2));
     arguments->request_handler = request_handler;
     arguments->client_fd = client_socket_number;
     pthread_create(&child, NULL, &thread_handle, (void *)arguments);
#elif POOLSERVER
    /* 
     * TODO: PART 7 BEGIN
     *
