...
def outputTour(shortestpathsDict, setMatrix, mapping, startingnode):
    ...
    for i in range(len(mapping)):
        if mapping[i]==startingnode:
            startindex=i
    matrix_sym = atsp_tsp(setMatrix, strategy="avg")
    outf = "/tmp/myroute.tsp"
    with open(outf, 'w') as dest:
        dest.write(dumps_matrix(matrix_sym, name="My Route"))
    tour = run(outf, start=startindex, solver="concorde")
    path=tour['tour']
...
for i in range(753,754):
    cur_file_name = "inputs/" + str(i) + ".in"
    cur_out_file = "outputs/" + str(i) + ".out"
    print(cur_file_name)
    print(cur_out_file)
