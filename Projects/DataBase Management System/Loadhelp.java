...
public class Loadhelp {
    public static String load(BufferedReader reader, String name, Database x) throws IOException {
        try {
            ...
            Table newTable = new Table(name, columnNames, columnTypes);
            ...
            while (nextline != null) {
                ...
                newTable.addRow(realNewRow);
                nextline = reader.readLine();
            }
            if (x.getDatabase().containsKey(name)) {  //Pls
                x.getDatabase().remove(name);
            }
            x.addTable(newTable);
            return "";
        }catch (RuntimeException ex){
            return "ERROR: WE have a malformed table";
        }
    }
}
